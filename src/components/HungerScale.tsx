...
export function HungerScale({ hunger, maxHunger, onChange, className }: HungerScaleProps) {
  return (
    <div className={cn('flex flex-col-reverse gap-1', className)}>
      <div className="flex items-center justify-center w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-muted text-muted-foreground">
      ...
      {Array.from({ length: maxHunger + 1 }).map((_, index) => {
        ...
        return (
          <button
            ...
            className={cn(
              'w-8 h-8 sm:w-10 sm:h-10 rounded border-2 flex items-center justify-center font-bold transition-all duration-200',
              'hover:scale-110 focus:outline-none focus:ring-2 focus:ring-ring text-xs sm:text-sm',
              active
